...
fn draw_video_frame(app: &mut MyApp, ui: &mut egui::Ui, draw_rect: egui::Rect) {
...
fn draw_telemetry_overlay(app: &mut MyApp, ui: &mut egui::Ui, draw_rect: egui::Rect) {
...
fn render_bottom_controls(app: &mut MyApp, ui: &mut egui::Ui, control_rect: egui::Rect) {
    ui.scope_builder(egui::UiBuilder::new().max_rect(control_rect), |ui| {
        ui.horizontal(|ui| {
            ...
            if start_ms_f > 0.0 {
                let r =
                    egui::Rect::from_x_y_ranges(rect.left()..=x_at(start_ms_f), y_range.clone());
                painter.rect_filled(r, 0.0, dim_color);
            }
            if end_ms_f < duration {
                let r = egui::Rect::from_x_y_ranges(x_at(end_ms_f)..=rect.right(), y_range.clone());
                painter.rect_filled(r, 0.0, dim_color);
            }
        });
    });
...
pub fn render_video_panel(app: &mut MyApp, ui: &mut egui::Ui) {
    egui::CentralPanel::default().show_inside(ui, |ui| {
        ...
        render_bottom_controls(app, ui, centered_controls);
    });
}
